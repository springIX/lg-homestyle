(() => {
  ...
  (function () {
    ...
    function initSwipersOnce() {
      ...
      const hasHearSignal = document.querySelector('#heart_signal_products .swiper'); // ✅ 추가

      if (!hasHearSignal) return;
      ...
      // 4) heart signal
      if (hasHearSignal) {
        $('#heart_signal_products article').each(function () {
          const swiperEl = $(this).find('.swiper')[0];
          const paginationEl = $(this).find('.pagi')[0];
          const prvEl = $(this).find('.prv')[0];
          const nxtEl = $(this).find('.nxt')[0];
