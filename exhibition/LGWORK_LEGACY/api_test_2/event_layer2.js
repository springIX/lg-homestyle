...
function bindCoupon() {
  ...
  $CAP(document).on("click.homeCoupon", ".homeCoupon", async function () {
    ...
    try {
      ...
      const failedCoupons = coupons.filter(c => c?.isIssue !== true);

      const normalizeMsg = (m) => String(m || '').trim();

      // 실패 사유 판별(메시지 기반)
      const isSoldOutFail = (msg) => /소진|마감|종료|재고\s*없|잔여\s*없/i.test(msg);
      const isAlreadyIssuedFail = (msg) => /이미|중복|기발급|발급\s*완료|다운로드\s*완료|이미\s*다운로드/i.test(msg);

      // 1) 하나라도 성공이면: 다운로드(또는 일부 다운로드)
      if (issuedCnt > 0) {
        // 필요하면 일부 다운로드 문구 유지
        if (failedCoupons.length > 0) {
          window.showAlert('일부 쿠폰이 다운로드 되었습니다.');
          return;
        }
        window.showAlert('쿠폰이 다운로드 되었습니다.');
        return;
      }

      // 2) 전부 실패면: "소진" vs "이미 다운받음" 구분
      const failMsgs = failedCoupons.map(c => normalizeMsg(c?.message));
      const hasSoldOut = failMsgs.some(isSoldOutFail);
      const hasAlready = failMsgs.some(isAlreadyIssuedFail);

      // ✅ 우선순위: 소진이 하나라도 있으면 '소진'을 우선 노출 (정책에 따라 반대로도 가능)
      if (hasSoldOut) {
        window.showAlert('쿠폰이 소진되었습니다.');
        return;
      }

      // ✅ 전부 실패인데 소진이 아니면 → "이미 받은 케이스"로 처리
      if (hasAlready) {
        window.showAlert('쿠폰이 다운로드 되었습니다.');
        return;
      }

      // 3) 그 외 실패(알 수 없는 메시지) → 기본값은 소진 or 오류 중 택1
      window.showAlert('쿠폰이 소진되었습니다.');
      return;
