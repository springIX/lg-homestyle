  ...
  class ProductCardUpdater {
    ...
    #updateProductCard(elItem, data, fallbackAttributes = null) {
      ...
      if (elements.badgeContainer) {
        ...
        // 3) ✅ 뱃지를 "최종가 li" 안으로 강제 배치 (복제+원본삭제 + 재시도)
        this.#forceMoveBadgeIntoFinalPrice(elItem, elements, elements.badgeContainer);
      }
 ...
 * - 원본 UL을 그대로 appendChild 하지 않음 (씹히는 케이스 대비)
 * - 최종가 li 안에 동일 UL(복제본)을 넣고, 원본 UL은 제거
 * - 혹시 DOM이 늦게 생성되는 경우 대비: 즉시 / 0ms / rAF / 50ms / 150ms / 300ms 재시도
 */
    #forceMoveBadgeIntoFinalPrice(elItem, elements, originalBadgeContainer) {
      const findFinalNode = () => {
        const priceArea = elements.priceArea || elItem.querySelector(ProductCardUpdater.SELECTORS.PRICE_AREA);
        ...
        // 2순위: ul.c-product__price-list 마지막 li
        finalNode = priceArea.querySelector('.c-product__price-list li:last-child');
        if (finalNode) return finalNode;

        // 3순위: (JSON 자동 생성 카드) ul.c-product__prices 중 숨김 아닌 것의 마지막 li
      ...
      const tryMove = () => {
        ...
        // 이미 이동된 뱃지가 있으면 종료
        if (finalNode.querySelector('.c-product__badge-container')) return true;

        // 중복 방지: finalNode 안에 뱃지 컨테이너가 있으면 제거
                    ...
                    <div class="c-product__inner">
                        ...
                        <div class="c-product__info-container has-margin-top--10">
                            ...
                            <div class="c-product__info has-padding-bottom--10 c-line c-line--bottom c-line--solid c-line--color-gray">
                                ...
                                <span class="c-product__model-id">${data["#Title"]}</span>
                            </div>
