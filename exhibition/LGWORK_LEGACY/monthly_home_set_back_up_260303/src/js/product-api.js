  ...
  class ProductCardUpdater {
    ...
    #updateProductCard(elItem, data, fallbackAttributes = null) {
      ...
      if (elements.priceArea) {
        elements.priceArea.innerHTML = this.#createPriceHTML(elItem, data);
      }

      // ✅ badgeContainer는 "정보영역(.inner) 하위" 것을 우선으로 잡음 (가장 안정적)
      const badgeContainer =
        elItem.querySelector('.ty_row .c-product__info-container .inner > .c-product__badge-container') ||
        elItem.querySelector('.ty_row .c-product__info-container .c-product__badge-container') ||
        elements.badgeContainer;

      if (badgeContainer) {
        // 1) 기존 할인 뱃지 제거
        badgeContainer.querySelector(ProductCardUpdater.SELECTORS.SALE_BADGE)?.remove();
        ...
        // 3) ✅✅✅ “이동”이 아니라 “복제+원본삭제”로 강제 배치
        this.#forceMoveBadgeIntoFinalPrice(elItem, elements, badgeContainer);
      }
    ...
    /**
     * ✅✅✅ 뱃지를 "최종가 li" 안으로 무조건 넣는 강제 함수
     * - 원본 UL을 그대로 appendChild 하지 않음 (씹히는 케이스 대비)
     * - 최종가 li 안에 동일 UL(복제본)을 넣고, 원본 UL은 제거
     * - 혹시 DOM이 늦게 생성되는 경우 대비: 0ms / rAF / 50ms / 150ms 재시도
     */
    #forceMoveBadgeIntoFinalPrice(elItem, elements, originalBadgeContainer) {
      const findFinalNode = () => {
        const priceArea =
          elements.priceArea ||
          elItem.querySelector(ProductCardUpdater.SELECTORS.PRICE_AREA);

        if (!priceArea) return null;

        // ✅ 1순위: 우리가 원래 노렸던 "최종가" li
        let finalNode = priceArea.querySelector('.c-product__price-item--final');
        if (finalNode) return finalNode;

        // ✅ 2순위: 가격 UL이 c-product__price-list 형태면 마지막 li를 최종가로 간주
        finalNode = priceArea.querySelector('.c-product__price-list li:last-child');
        if (finalNode) return finalNode;

        // ✅ 3순위: (JSON 자동 생성 카드처럼) ul.c-product__prices 형태면,
        // 숨김 아닌 ul 중 "마지막 li"를 최종가로 간주
        finalNode = priceArea.querySelector('.c-product__prices:not(.hidden-all) li:last-child');
        if (finalNode) return finalNode;

        // ✅ 4순위: 정말 예외 구조면 priceArea 내부 li 중 마지막
        finalNode = priceArea.querySelector('li:last-child');
        return finalNode || null;
      };

      const tryMove = () => {
        const finalNode = findFinalNode();
        if (!finalNode) return false;

        // 이미 이동(생성)된 뱃지 있으면 종료
        if (finalNode.querySelector('.c-product__badge-container')) {
          return true;
        }

        // 중복 방지: finalNode 안에 뱃지가 있으면 제거
        finalNode.querySelectorAll('.c-product__badge-container').forEach((n) => n.remove());

        // ✅ "이동"이 아니라 "복제+원본삭제"로 강제 반영
        const clone = originalBadgeContainer.cloneNode(true);
        clone.dataset.moved = '1'; // 디버그용 (원하면 삭제해도 됨)

        finalNode.appendChild(clone);

        // 원본 제거
        if (originalBadgeContainer.isConnected) originalBadgeContainer.remove();

        return true;
      };

      // 즉시 + 지연 재시도 (DOM 갱신/치환 케이스 대응)
      if (tryMove()) return;
    ...
    #createPriceHTML(elItem, data) {
        ...
        return `
                        <ul class="c-product__price-list c-product__price-list--subscription">
                            ...
                            <li class="c-product__price-item c-product__price-item--final">
                                ...
                                <span class="c-product__price-value c-product__price-value--final"><span class="c-product__unit">월</span> ${format(rentalModelInfo.maxBenefitPrice)}<span class="c-product__unit">원</span></span>
                            </li>
                        </ul>
                        `;
      }
      ...
      return `
                    <ul class="c-product__price-list c-product__price-list--one-time-purchase">
                        ...
                        <li class="c-product__price-item c-product__price-item--final">
                            <span class="c-product__price-label c-product__price-label--final">최대혜택가</span>
                            <span class="c-product__price-value c-product__price-value--final"><strong>${format(priceInfo.lastBenefitPrc)}</strong><span class="c-product__unit">원</span></span>
                        </li>
                    </ul>
                    `;
    }
