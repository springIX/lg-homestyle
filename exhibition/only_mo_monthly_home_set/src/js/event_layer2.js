...
function bindCoupon() {
  ...
  $CAP(document).on("click.homeCoupon", ".homeCoupon", async function () {
    ...
    try {
      ...
      // ✅ [수정] 실패 케이스 분기 (소진 먼저 → 개인한도)
      const hasSoldOut = coupons.some(c => {
        const msg = c?.message || '';
        return c?.isIssue === false && (
          msg.includes('총발급수량') ||  // ✅ 소진 케이스 (2026-02-05 11:04 응답)
          msg.includes('소진')
        );
      });

      if (hasSoldOut) {
        window.showAlert('이미 소진된 쿠폰입니다.');
        return;
      }

      const hasAlreadyIssued = coupons.some(c => {
        const msg = c?.message || '';
        return c?.isIssue === false && (
          msg.includes('개인별(') ||     // ✅ 이미 발급(개인별) 케이스 (2026-02-05 11:09 응답)
          msg.includes('발급한도[')
        );
      });

      if (hasAlreadyIssued) {
        window.showAlert('이미 발급 받은 쿠폰입니다.');
        return;
      }

      // 여기까지면 기타 실패(원하면 문구 바꿔도 됨)
      window.showAlert('다운로드에 실패했습니다.');
      return;
