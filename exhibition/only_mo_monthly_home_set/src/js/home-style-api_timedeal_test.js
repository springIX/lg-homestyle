(() => {
  class HomeStyleProductApi {
    constructor() {
      ...
      this.endpoints = {
        ...
        timeDeals: '/displaysvc/ajax/v1/time-deal/goods'
      };
    }
  ...
  class HomeStyleProductUIController {
    ...
    async #updateProducts() {
      ...
      try {
        if (this.#useMockData) {
          ...
          (timeDeals || []).forEach((item) => {
            if (normalProductIds.includes(item.productId)) {
    ...
    `;
      ...
      if (elements.priceArea && data.price) {
        const timeDealData = this.#timeDealDataMap.get(data.goodsNo);
        ...
        elements.priceArea.innerHTML = this.#createPriceHTML(priceData);
      }

      this.#renderTimeDealTimer(
        elItem,
        this.#timeDealDataMap.get(data.goodsNo)
      );
    ...
    #renderTimeDealTimer(elItem, timeDealData) {
      ...
      timer.dataset.startDate =
        timeDealData.startDate;

      timer.dataset.endDate =
        timeDealData.endDate;
  ...
  `;
     ...
     * 종료 시 scaleX(0)
     */
    #refreshTimeSaleProgress(
      startDateValue,
      endDateValue
    ) {
      const progress =
        document.querySelector(
          '#timesale .timesale_progress'
        );

      const progressBar =
        progress?.querySelector(':scope > i');
