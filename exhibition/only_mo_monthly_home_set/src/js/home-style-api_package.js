(() => {
  class HomeStyleProductApi {
    ...
    async fetchPackages(goodsNos) {
      ...
      console.log('[fetchPackages raw response]', response);
  ...
  class HomeStyleProductUIController {
    ...
    #updateProductCard(elItem, data) {
      ...
      if (elements.imageWrapper) {
        const thumbUrl = elements.mainSpan.dataset.productThumb;
        const imageUrl = data.imgPath || '';

        elements.imageWrapper.innerHTML = `
          <img src="${thumbUrl || `${this.apiService.imageBaseUrl}${imageUrl}?aw=1&ah=1&rw=500&rh=500`}"
              alt="${data.imgAlt || ''}"
  ...
  class HomeStylePackageUIController {
    constructor() {
      this.apiService = new HomeStyleProductApi();
      this.init();
    }
    ...
    async init() {
      const packageEls = document.querySelectorAll('[data-home-style-package]');
      if (!packageEls.length) return;

      const packageNos = [...packageEls]
        .map(el => el.dataset.homeStylePackage)
        .filter(Boolean);

      const packageList = await this.apiService.fetchPackages(packageNos);

      console.log('[packageList]', packageList);
      console.log('[packageList full]', JSON.stringify(packageList, null, 2));
      // 여기부터 패키지 전용 렌더링 추가 예정
    }
  }
  ...
  (function () {
    ...
    function initSwipersOnce() {
      ...
      const hasHowToApply = document.querySelector('#how_to_apply .swiper');
      const hasAiHomeStyling = document.querySelectorAll('#ai_home_styling .swiper');
      const hasHowTobuy = document.querySelector('#how_to_buy .swiper');
      ...
      if (hasAiHomeStyling.length) {
        hasAiHomeStyling.forEach(swiper => {
          console.log("d" + swiper);
          new Swiper(swiper, {
            slidesPerView: 1,
            observer: true,
            observeParents: true,
            scrollbar: {
              el: swiper.querySelector('.scr_bar'),
              draggable: true,
            },
          });
          console.log('created');
    ...
    function initHomeStyle() {
      ...
      const productTargets = document.querySelectorAll('[data-home-style-product]');
      const packageTargets = document.querySelectorAll('[data-home-style-package]');

      if (!productTargets.length && !packageTargets.length) {
        console.log('[HomeStyle] product/package DOM not found yet');
        return;
      }

      initialized = true;
      console.log('[HomeStyle] init HomeStyleProductUIController');
      if (productTargets.length) new HomeStyleProductUIController();
      if (packageTargets.length) new HomeStylePackageUIController();
