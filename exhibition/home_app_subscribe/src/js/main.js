$(function () {
  ...
  if ($cardSection.length) {
    ...
    const cardInfo =
      new Swiper(
        '#detail-affiliated-card .card-info',
        {
          ...
          navigation: {
            nextEl:
              '#detail-affiliated-card .nxt',

            prevEl:
              '#detail-affiliated-card .prv'
          },
    ...
    /*
     * 모든 카드 notice 아코디언 닫기
     */
    function closeCardAccordions() {
      const $accordions =
        $cardSection.find(
          '.card-info .notice.accordion'
        );
